...
- `object_date` / `object_date_to` / `object_date_precision` (optional — the object's own creation date, carrying its own precision through the same `date_precision` vocabulary and circa margin as a transfer date. Most of what a museum's object records hold is imprecise, so a field accepting only exact ISO dates would push "ca. 1905" back into the unrecorded bucket it exists to shrink; only the *widened* earliest day counts, so `circa 1948` does not step the rule aside. **Rule 4's absent-chain limb consults it too** — that limb makes the same inference rule 1 does, and it inverts for the same reason: for an object created after the band an absent pre-1945 chain is arithmetic, not the trace of a confiscation channel. **Added 2026-08-14 after external review.** Rule 1 fires whenever nothing in the chain predates 1945, which for an object created in 1962 is correct by the rule and useless in practice — and it lands on the coverage map, the first screen. Where the recorded creation date is on or after 1945 the rule reports itself *not applicable* with the reason; where it is blank the rule still runs, because an unrecorded date is not evidence the object is modern and disabling the modal rule on an empty column is the silent false negative this spec exists to prevent. The coverage map counts `created_after_the_risk_band` and `creation_date_not_recorded` separately from `with_no_pre_1945_provenance`. Precision is granularity-derived and only the earliest possible day counts.)
...
**`entry_kind` is declared, never inferred (resolved 2026-08-14).** Both bundled lists now carry `entry_kind: person | organisation` on every entry, and both loaders refuse to default it exactly as they refuse to default `implies_persecution_of_former_owner`. Two separate defects were the same guess going wrong in opposite directions: a compound surname (`von Behr`) read as Given+Surname, and an auction house rendered with "the list entry names an individual" — an untrue sentence in the one rendering path whose whole justification is legal accuracy. Nine entries to annotate now; two thousand later, after the guess had been wrong in public. The kind governs wording only: whether a match is identity-confirmed still turns on whether the entry knows any individual by that name.
...
**`transaction_state` engages rule 3 directly, and `fluchtgut` has its own criterion (resolved 2026-08-14).** The REAO-grounded state field — described above as the correction that grounds the whole taxonomy — drove no rule of its own. It was read to pick a tier *inside* rule 3's territory-window gate, to suppress settled records, and as the independent-establishment check on the actor gate; so a state could only ever select a tier it could not itself reach. A recorded `zwangsverkauf` in Geneva returned "screened against N criteria; no criteria triggered", with rule 3 listed as *evaluated* rather than skipped, while an ordinary purchase in Munich flagged. That is the terminal state the Output section's no-score reasoning exists to prevent, reached by a different route.

**The tier derivation on limb 2 declines rather than defaults (corrected 2026-08-14, same day).** The first cut clipped the record's span to the risk band and fell through to a default, which was wrong with both signs: a 1970 record produced an empty intersection and got the *milder* tier with a basis string claiming its position was undetermined when it was not, while a `circa 1935` record got the *harsher* tier with a basis string that said undetermined. Both are the same defect — picking a tier where the honest answer is that there isn't one — and defaulting to the milder tier on unplaceable data is the false-negative direction besides. Limb 2 now tests the *widened* span against the threshold directly: wholly on or after it is heightened, wholly before it is ordinary, and anything else asserts **no tier** with the reason. A date outside the screening band asserts no tier either and says the record's state and date do not agree; the flag still stands, because the state still records a persecution transfer. A test asserts the invariant behind both bugs: the basis never contradicts the tier it explains.

Rule 3 now has a second limb: a record whose own `transaction_state` is `entziehung`, `zwangsverkauf`, `vermutung_der_entziehung` or `verschaerfte_vermutung` engages it regardless of territory, with `presumption_tier_basis` saying the tier came from the record and not from a window, and no `territory` field claimed. Where the state carries no intrinsic tier and the record has no date, the flag asserts **no tier** rather than picking one. A record already flagged by limb 1 is not flagged twice.
...
- **CI enforces the offline checks** (`.github/workflows/checks.yml`): the test suite and `check_publishable.py`, blocking, on every push and pull request. `fetch-depth: 0` is load-bearing — the history scan passes vacuously against a shallow clone, reporting "clean" because it can only see one commit, which is worse than not running it. **No workflow may hold a credential or call the Infomaniak API**; that is the whole reason the demo is a committed static file.
- **The live guard validation cannot be automated and is not pretended otherwise.** CI enforces a proxy: a pull request touching `llm_guard.py`, `llm_client.py`, the language pack or the harness must also touch `README.md`, so the validation run table moves with the guard. The failure message says in terms that this checks whether a human wrote down that the run happened, not whether it did.
- **The dead config file is now loud rather than silent.** `config.example.yaml` is a reference sheet for the CLI flags — nothing reads YAML — and the risk was never the file but the silence: a user copies it to `config.yaml`, edits it, and the tool runs on defaults while they believe otherwise. The pipeline now warns when a `config.yaml` exists, every setting names the flag that controls it, and a test asserts both, so the two cannot drift.
- **`CONTRIBUTING.md` states what the project actually needs**, which is methodology review rather than code, along with the six non-negotiable design constraints and the limits of what CI can check. Issue templates route a wrong *flag* to a methodology template and a wrong *behaviour* to a bug template — different failures needing different people — and the bug template asks explicitly for synthetic input, because a real provenance record in a public tracker is a disclosure of personal data.

Two review points were accepted without a fix, because they are accurate descriptions rather than defects: the bus factor of one, and the seed lists' non-match carrying almost no information. The second is already surfaced in every `coverage_note` and above the queue in the report; the residual risk is a reader who skims past it, which no amount of code removes.
